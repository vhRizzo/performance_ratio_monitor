 ...
 * @param pin_tx_uart Pino TX do ESP32 p/ enviar ao Modem: Padrão #define
 * TX_SERIAL_AT_PIN (GPIO_NUM_14)
 * @param pin_rx_uart Pino RX do ESP32 p/ receber do Modem: Padrão #define
 * RX_SERIAL_AT_PIN (GPIO_NUM_13)
 * @param baud_rate Taxa sinalização Modem: Padrão 9600 bps (conforme datasheet)
 * @param data_bits Bits de dados por ciclo: Padrão UART_DATA_8_BITS (conforme
 * datasheet)
 ...
 */
Modem_SmartModular::Modem_SmartModular(uart_port_t uart_num,
                                       uint8_t pin_tx_uart, uint8_t pin_rx_uart,
                                       uint32_t baud_rate, uint8_t data_bits,
    ...
    // ira automaticamente bloquear um TX até terminar a transmissão
    // (https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/uart.html#_CPPv419uart_driver_install11uart_port_tiiiP13QueueHandle_ti)
    ESP_ERROR_CHECK(
        uart_driver_install(uart_tx_rx, tam_rx, tam_tx, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(uart_tx_rx, &uart_config));
    ESP_ERROR_CHECK(uart_set_pin(uart_tx_rx, pin_tx_uart, pin_rx_uart,
                                 UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
#ifdef SERIAL_DEBUG
    ESP_LOGI(__func__, "UART: %u,%u,%u,%lu,%u,%u,%u,%u,%lu", uart_num,
             pin_tx_uart, pin_rx_uart, baud_rate, data_bits, parity, stop_bits,
             flow_control, tam_buffer);
#endif
    // xTaskCreate(&process_wrapper, "rx_task", 2048, NULL,
    // configMAX_PRIORITIES-5, &xHandle);(void*)&this
    xTaskCreate(&process_wrapper, "rx_task", 2048, (void *)this,
                configMAX_PRIORITIES - 5, &xHandle);
}
 ...
 * @return char* contém qual o valor do DEUI ou APPEUI ou APPKEY conforme
 * comando enviado
 */
...
#endif
    while (comando_ok == 0) {
        uart_write_bytes(uart_tx_rx, comando,
                         strlen(comando));  // reiniciar a MCU do modem para
                                            // enviar comando de novo
#ifdef SERIAL_DEBUG
        ESP_LOGI(__func__, "Comando enviado! Esperando modem...");
#endif
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        do {
            while (xQueueReceive(resposta_AT, (void *)rx_buf,
...
// ESP_LOGI(__func__, "COMANDO EXECUTADO COM SUCESSO...");
                ...
                contar_erros++;
                if (contar_erros > 20) esp_restart();
            }
        ...
        if (comando_ok == 1) {
            ...
            token = strtok(rx_buf, "\r\n");
            return token;  // retorna a resposta do comando (o próximo token
                           // seria o OK...)
        }
    }
#ifdef SERIAL_DEBUG
...
/**
 * @brief Enviar comando de AJOIN para realizar JOIN automaticamente após a
 * inicialização do módulo Porém, é necessário ter definido as APPEUI (método
 * OTAA_APPEUI) e APPKEY (método OTAA_APPKEY) e depois salvá-las com com método
 * save_config
 * @param valor -1 (default sem parâmetro retorna se está ou não com AJOIN
 * atividado), 0 desativa e 1 ativa
...
#ifdef SERIAL_DEBUG
    ESP_LOGI(__func__,
             "Parâmetro incorreto, deve ser 1 para ligar o auto join, ou 0 "
             "para desabilitar.");
#endif
    return NULL;
}
...
/**
 * @brief Salva as configurações feitas para que possam ser usadas após eventual
 * reinicialização
 *
 ...
 */
char *Modem_SmartModular::OTAA_DEUI() {  // vetor contendo 8 bytes de DEVEUI
                                         // separados por ':' e terminado por
 ...
 *
 * @param APPEUI string contendo 8 bytes do APPEUI separados por ':' , se for
 * NULL, o método retorna o APPEUI gravado na placa do radio
 * @return char* retorna a APPEUI gravado na placa do radio
 */
char *Modem_SmartModular::OTAA_APPEUI(
    char *APPEUI) {  // vetor contendo 8 bytes em 00 de APPEUI separados por ':'
                     // e terminado por '\n' (padrão ChirpStack). Tamanho do
                     // vetor: 16 bytes
    char comando_listar[14] = "AT+APPEUI=?\r\n";
    char comando_appeui[37] =
        "AT+APPEUI=00:00:00:00:00:00:00:00\r\n";  // padrão chirpstack...
// para analise relacionado ao sprintf abaixo|
// "AT+APPEUI=00:00:00:00:00:00:00:00\r\n"; adotar padrão chirpstack.... mas se
// precisar....
#ifdef SERIAL_DEBUG
...
#ifdef SERIAL_DEBUG
            ESP_LOGI(__func__,
                     "APPEUI além 23 bytes de \"00:00:00:00:00:00:00:00\" , "
                     "tamanho: %i...",
                     strlen(APPEUI));
#endif
            return NULL;
 ...
 *
 * @param APPKEY string contendo 16 bytes do APPKEY separados por ':' , se for
 * NULL, o método retorna o APPKEY gravado na placa do radio
 * @return char* retorna o APPKEY gravado na placa do radio
 */
char *Modem_SmartModular::OTAA_APPKEY(
    char *APPKEY) {  // vetor contendo 16 bytes de APPKEY separados por ':' e
...
char *Modem_SmartModular::versao() {  // vetor contendo 16 bytes de DEVEUI
                                      // separados por ':' e terminado por '\n'.
                                      // Tamanho do vetor: 32 bytes
...
/**
 * @brief Método para enviar comando de RESET ao Modem. Se não retornar OK após
 * 20 tentativas, força ESP restart...
 *
 */
...
#endif
    while (resetou == 0) {
        uart_write_bytes(uart_tx_rx, cmd_reset,
                         strlen(cmd_reset));  // reiniciar a MCU do modem para
                                              // enviar comando de novo
#ifdef SERIAL_DEBUG
...
/**
 * @brief Método para enviar comando de JOIN para o Modem. Faz uso do método
 * this->join() para checar se o JOIN foi efetivado com sucesso, devido ao tempo
 * imprevisível que possa levar para fazer o JOIN. Esse método aciona o método
 * reset() após 5 tentativas de JOIN, uma vez que a comunicação ficou sem acesso
 * à rede LoraWAN...
 *
...
#endif
                        vTaskDelay(5000 /
                                   portTICK_PERIOD_MS);  // tempo máximo para
                                                         // fazer Join...
                        xQueueReset(resposta_AT);  // zerar vestigios de JOINED
                                                   // não documentado...
...
#ifdef SERIAL_DEBUG
                        ESP_LOGI(__func__, "Resposta não prevista...: %s",
                                 rx_buf);
#endif
                    }
                    contar_erros = 0;
...
/**
 * @brief Envia comando ao Modem de verificação se fez JOIN na rede Lorawan. Há
 * 20 tentativas, se não conseguir, reinicia o mode. Ainda, se por algum motivo
 * o Modem travar e o ESP32 também, watchdog reiniciará ESP32...
 *
...
/**
 * @brief Task permanentemente em execução para monitorar o RX da UART por dados
 * do Modem e atribuindo esses dados recebidos em String para a queue respostaAT
 *
 * @param pvParameters Terá o ponteiro para o Objeto instanciado dessa classe
 * (obtido do método static process_wrapper...)
 */
void Modem_SmartModular::rx_task(void *pvParameters) {
    ...
    while (1) {
        ...
        while (rxBytes != -1) {
            rxBytes = uart_read_bytes(ptr->uart_tx_rx, (void *)&tmp[i], 1,
...
#endif
                if (tmp[i] == '\n' ||
                    tmp[i] == '\r') {  // Segundo manual smartmodular, após '\r'
                                       // o último é '\n'. Porém inconsistencia
                                       // com JOINED nao documentado (por isso
                                       // tmp[i]=='\r')
                    int tam_rx_ring = 0;
                    ESP_ERROR_CHECK(uart_get_buffered_data_len(
                        ptr->uart_tx_rx, (size_t *)&tam_rx_ring));
#ifdef SERIAL_DEBUG
...
#endif
                        while (xQueueSend(ptr->resposta_AT, (void *)tmp,
...
/**
 * @brief Método para Enviar dados para a nuvem e Receber, se existir downlink,
 * da nuvem. Os dados tx e rx são vetores codigicados em caracteres ASCII e em
 * formato string.
 *
 * @param porta Indica qual a porta de uplink, que será referência para
 * downlink.
 ...
 * @param tam_dados_rx Endereço de variável onde armazenará o Tamanho da String
 * recebida. Se conter 0, não houve downlink...
 */
void Modem_SmartModular::enviar_receber(uint16_t porta, char *dados_tx,
                                        uint32_t tam_dados_tx, char *dados_rx,
                                        uint32_t *tam_dados_rx) {
    uint8_t contar_erros = 0;
    ...
    tx_buf[tam_msg_total++] =
        '\r';  // montar a string padrão AT a enviar na UART
    tx_buf[tam_msg_total++] = '\n';
...
// ESP_LOGI(__func__, "AT+SEND retornou: %s", rx_buf);
            ...
            if (strstr(rx_buf, "AT_ERROR") != NULL) {
                ...
                if (strstr(rx_buf, "OK") !=
                    NULL) {  // BUG se +EVT , pode não vir OK e pode-se perder
                             // dados no RECV
#ifdef SERIAL_DEBUG
    ...
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    // xQueueReset(resposta_AT); //limpar qualquer lixo na fila vindo do SEND ou
    // +EVT;
...
#endif
        uart_write_bytes(
            uart_tx_rx, comando_recv,
            strlen(comando_recv));  // pegar o que foi recebido RX1 e RX2
        vTaskDelay(1000 / portTICK_PERIOD_MS);
#ifdef SERIAL_DEBUG
...
#endif
        if (token2 != NULL) {
            if ((uint8_t)strtol(token2, &tmp, 10) ==
                porta) {  // só entra se achar a porta
                token2 =
                    strtok(NULL, ":");  // após a porta: pode conter os dados...
                if (token2 != NULL &&
                    strlen(token2) > 0) {  // contem downlink...
#ifdef SERIAL_DEBUG
...
/*
void Modem_SmartModular::enviar_receber_bytes(uint16_t porta, uint8_t *dados_tx,
uint32_t tam_dados_tx, uint8_t *dados_rx, uint32_t *tam_dados_rx){ uint32_t
tam_msg_total = 0; uint8_t enviado=0;
    ...
    //for(int i=0;i<tam_dados_tx;i++){ //ajuste com base na tabela ASCII (byte
para valor em hexa na string)
                                     //Ex: decimal 55, seria 37 em hexa. Ficaria
7 em string. Ficará em texto 3337, no ASCII p/ modem 37... for(int
i=tam_dados_tx-1;i>=0;i--){ uint8_t tmp_1o_nibble = dados_tx[i] & 0x0F; uint8_t
tmp_2o_nibble = dados_tx[i] >> 4;

        if(tmp_2o_nibble >= 0x0 && tmp_2o_nibble <= 0x9) //0 até 9, ficará de
0x30 - 0x0x39 tx_buf[tam_msg_total]=tmp_2o_nibble + 0x30; else //A até F, ficará
de 0x41 - 0x0x46
        //if(tmp_2o_nibble >= 0xa && tmp_2o_nibble <= 0xf)
            tx_buf[tam_msg_total]=tmp_2o_nibble + 0x37;
        tam_msg_total++;
        if(tmp_1o_nibble >= 0x0 && tmp_1o_nibble <= 0x9) //0 até 9, ficará de
0x30 - 0x0x39 tx_buf[tam_msg_total]=tmp_1o_nibble + 0x30; else //A até F, ficará
de 0x41 - 0x0x46
    ...
    while(enviado==0){
        uart_write_bytes(uart_tx_rx, tx_buf, tam_msg_total);
        vTaskDelay(3000 / portTICK_PERIOD_MS); //tempo da janela RX1 (1s) e RX2
(2s) while(xQueueReceive( resposta_AT, ( void * ) rx_buf, ( TickType_t )
1000)!=pdTRUE){} #ifdef SERIAL_DEBUG ESP_LOGI(__func__, "AT+SENDB retorno: %s",
(char *) rx_buf); #endif if(strstr((char *)rx_buf, "AT_ERROR")!=NULL){
            vTaskDelay(1000 / portTICK_PERIOD_MS);
            continue;
        }
    ...
    while(recebido==0){
        uart_write_bytes(uart_tx_rx, comando_recv, strlen(comando_recv)); //não
inclui o \0 vTaskDelay(1000 / portTICK_PERIOD_MS); while(xQueueReceive(
resposta_AT, ( void * ) rx_buf, ( TickType_t ) 1000)!=pdTRUE){} #ifdef
SERIAL_DEBUG ESP_LOGI(__func__, "AT+RECV retornou:%s", (char*) rx_buf); #endif
    ...
    *tam_dados_rx = strlen((char *) rx_buf); //o 1o token será porta:dados , que
é o importante, depois vira \r\nOK\r\n não usaveis aqui.
