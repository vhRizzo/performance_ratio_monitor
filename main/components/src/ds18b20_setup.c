...
#include "ds18b20_setup.h"  // Includes moved to header -N
...
QueueHandle_t ds18b20_queue;  // Handler for the queue to send data -N
...
bool ds18b20_setResolution(const DeviceAddress tempSensorAddresses[],
    ...
    for (int i = 0; i < numAddresses; i++) {
        ...
        if (ds18b20_isConnected((DeviceAddress *)tempSensorAddresses[i],
                                scratchPad)) {
            ...
            if (scratchPad[CONFIGURATION] != newValue) {
                ...
                ds18b20_writeScratchPad((DeviceAddress *)tempSensorAddresses[i],
                                        scratchPad);
            }
            // done
            success = true;
...
void ds18b20_writeScratchPad(const DeviceAddress *deviceAddress,
...
bool ds18b20_readScratchPad(const DeviceAddress *deviceAddress,
                            uint8_t *scratchPad) {
...
void ds18b20_requestTemperatures() {
    ...
    while (!isConversionComplete() &&
           ((esp_timer_get_time() / 1000ULL) - start <
...
bool ds18b20_isConnected(const DeviceAddress *deviceAddress,
                         uint8_t *scratchPad) {
    bool b = ds18b20_readScratchPad(deviceAddress, scratchPad);
    return b && !ds18b20_isAllZeros(scratchPad) &&
...
float ds18b20_getTempF(const DeviceAddress *deviceAddress) {
    ...
    if (ds18b20_isConnected(deviceAddress, scratchPad)) {
        ...
        if (rawTemp <= DEVICE_DISCONNECTED_RAW) return DEVICE_DISCONNECTED_F;
        // C = RAW/128
...
int16_t calculateTemperature(const DeviceAddress *deviceAddress,
                             uint8_t *scratchPad) {
...
float ds18b20_get_temp(void) {
    if (init == 1) {
        ...
        if (check == 1) {
            ...
            // portTICK_RATE_MS is deprecated -N
...
void ds18b20_init(int GPIO) {
    ...
    esp_rom_gpio_pad_select_gpio(
        DS_GPIO);  // This was slightly changed in esp-idf v5.0 update -N
    init = 1;
}
...
/* Task to collect the sensor's data and send it through a FreeRTOS queue.
 * Everything in this task is new, so the "-N" identifier will be omitted. */
void ds18b20_task(void *pvParameters) {
    // Initializes the sensor
    ds18b20_init(DS18B20_PIN);
    // Initializes the item to be sent through the queue
    ds18b20_t dallas = {.dallas_temp = -1};
    while (true)  // Loops infinitely
    {
        vTaskDelay((TEMPO_ANALISE * 1000) / portTICK_PERIOD_MS);
        // Get a new temperature reading
        dallas.dallas_temp = ds18b20_get_temp();
        // And send it through the queue
